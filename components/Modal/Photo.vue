<template>
  <Modal>
    <div
      class="tw-w-fit tw-h-fit tw-bg-white tw-rounded-lg tw-relative tw-overflow-auto"
    >
      <nav
      ...
      <section class="tw-relative">
        <video ref="video" @canplay="initCanvas()" width="960">
          Stream Unavailable
        </video>
        <button
          @click="takePicture()"
          class="tw-absolute tw-p-6 tw-bg-red-500 tw-right-4 tw-top-1/2 -tw-translate-y-1/2 tw-rounded-full"
        ></button>
...
const takePicture = () => {
  ...
  imageStore.images.push({
    ...
    name: `image ${localStorage.getItem("images")?.length}`,
  });
